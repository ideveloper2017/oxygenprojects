  ...
  public async payment_sum_in(town_id:number,paymentmethods:string,caisher_id:number){
    let res;
     res=  await this.orderRepo.manager.createQueryBuilder(Payments,'payments')
        ...
        .where('(payments.caisher_type= :cash) and (town.id= :town_id) ' +
            '         and (caishers.id= :caisher_id) and (payments.paymentmethods= :paymentmethods)',{cash:Caishertype.OUT,
                          town_id:town_id,
                          caisher_id:caisher_id,
